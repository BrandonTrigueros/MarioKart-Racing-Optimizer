...
double MotorcycleDriver::get_time_land (int distancia) {
    ...
    return (((0.4 * distancia) * (this->VehicleD->getAceleration() + this->VehicleD->getBaseSpeed()))
...
double MotorcycleDriver::get_time_air (int distancia) {
    if (distancia == 0 ) return 0;
    return (((0.5 * distancia) * (this->VehicleD->getAceleration() + this->VehicleD->getBaseSpeed()))
